...
This is off-by-one from how *every other modern compiler* counts bootstrapping.
...
To make the new flags esaier to learn, we can name the sysroots in `build/host` after the flags: `build/host/{bootstrap,dev,dist}-sysroot`.
